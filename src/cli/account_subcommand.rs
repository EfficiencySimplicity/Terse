...
impl AccountSubcommand {
    pub fn process(self, server_list: &mut ServerList) -> Result<(), Error> {
        match self {
            Self::New                           => Self::process_new(server_list)?,
            Self::Login  { username, password } => Self::process_login(server_list, username, password)?,
            Self::Logout { username }           => Self::process_logout(server_list, username)?,
            Self::Delete { username, }          => Self::process_delete(server_list, username)?,
    ...
    fn process_login(server_list: &mut ServerList, username: String, password: String) -> Result<(), Error> {
    ...
    fn process_logout(server_list: &mut ServerList, username: String) -> Result<(), Error> {
        server_list.selected()?.remove_account(&username)?;
        println!("I successfully logged you out of the account known as {username} :)");
        Ok(())
    }
